...
/* The spiral traced by hand for the Snail, kept at the precision it was drawn
   at and shared with the Whelk, which is the same line under different
   forces. */
const SPIRAL = { id: 's11nd1', name: 'strand 1', closed: false, pts: [
...
const PRESET_LIST = [
  ...
  {
    name: 'Whelk', group: 'growth',
    note: 'The Snail again — the same traced spiral, the same forces — with the repulsion graded by how far apart two nodes are along the line rather than only by how far apart they are in space. A node and its near neighbours barely push, so the line stays free to bend; a fold meeting another fold pushes at full strength, so the folds stand well off each other. The mean gap between passes goes from 61 to 89 and the whole sheet opens out.',
    cfg: { seeds: [{ key: 'drawn:s11nd1', dx: 0, dy: 0, rot: 0, scale: 1 }],
           // full strength twelve nodes apart, nothing at zero. Much past this
           // and the near-neighbour push that drives growth is gone with it
           repulsionRamp: 12,
           maxNodes: 20000, pauseAtBudget: false, settleAt: 0.04, maxSteps: 2000,
           fillOn: false, follow: true,
           drawnShapes: [SPIRAL] },
  },
  {
